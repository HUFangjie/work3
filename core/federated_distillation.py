...
# Paths / CSV helpers
# =========================================================
def _get_artifact_dir(config: Dict) -> str:
    """Return the directory used for run artifacts such as CSVs and logs."""
    log_cfg = config.get("logging_config", {})
    artifact_dir = log_cfg.get("artifact_dir") or log_cfg.get("log_dir", "./logs")
...
def _get_metrics_csv_path(config: Dict) -> str:
    log_cfg = config.get("logging_config", {})
    log_dir = _get_artifact_dir(config)
